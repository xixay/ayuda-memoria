# 📚 Índice de Contenidos

---

## 1. 🖥️ Sistema y Herramientas

| Tema | Enlace | Tema | Enlace |
|------|--------|------|--------|
| **1.1. Instalar programas .deb** | [Ver](/1inicio/1-instalarTipoDeb.md) | **1.2. Actualización terminal (Linux)** | [Ver](/1inicio/2-actualizarSistema.md) |
| **1.3. Vista en terminal (Oh My Zsh + Ubuntu)** | [Ver](/1inicio/3-ZSHeInstalarPowerlevel10k.md) | **1.4. Instalar Node (Linux)** | [Ver](/1inicio/4-node.md) |
| **1.5. GIT** | [Ver](/1inicio/5-git.md) | **1.6. GIT (Zsh abreviado)** | [Ver](/1inicio/6-gitAbreviado.md) |
| **1.7. Formato de commits** | [Ver](/1inicio/7-commit.md) | **1.8. Crear repositorio (Github)** | [Ver](/1inicio/8-repositorio.md) |
| **1.9. Claves SSH (Github/Gitlab)** | [Ver](/1inicio/9-clavesSshLinuxWindowsWsl2.md) | **1.10. Editar archivos (Linux)** | [Ver](/1inicio/10-abrirYEditarArchivosSistema.md) |
| **1.11. Configuración global Git (usuario/email)** | [Ver](/1inicio/11-agregarConfigUsserEmail.md) | **1.12. Docker (Windows, Linux, WSL2)** | [Ver](/1inicio/12-docker.md) |
| **1.13. TMUX** | [Ver](/1inicio/13-tmux.md) | **1.14. Notas en árbol** | [Ver](/1inicio/14-todo-tree.md) |
| **1.15. Color en VSCode (Peacock)** | [Ver](/1inicio/15-peacock.md) | **1.16. GitIgnore** | [Ver](/1inicio/16-gitignore.md) |
| **1.17. Base de Datos (Intro)** | [Ver](/1inicio/17-postgresql.md) | **1.18. Tema VSCode** | [Ver](/1inicio/18-temas.md) |
| **1.19. Grabador de Video (Kasan)** | [Ver](/1inicio/19-Kasan.md) | **1.20. JavaScript (intro)** | [Ver](/1inicio/20-javaScript/readme.md) |
| **1.21. VSCode Configuración** | [Ver](/7archivos/perfil/21-vscode-config.md) | **1.22. Debugger** | [Ver](/1inicio/22-debugger.md) |
| **1.23. Matar puertos** | [Ver](/1inicio/23-matarPuertos.md) | **1.24. Snippets** | [Ver](/1inicio/24-snippets.md) |
| **1.25. Obsidian** | [Ver](/1inicio/25-obsidyan.md) | **1.26. Google Drive + Obsidian** | [Ver](/1inicio/26-google_drive_obsidian.md) |
| **1.27. GitHub Desktop** | [Ver](https://github.com/xixay/GitHub-Desktop) |  **1.28. Warp terminal IA** | [Ver](/1inicio/27-terminal_warp_con_ia.md)

---

## 2. ⚙️ Backend

### 2.1. NestJS

| Tema | Enlace | Tema | Enlace |
|------|--------|------|--------|
| **A.1. Variables de entorno** | [Ver](/2backend/nestJs/A.1-Env.md) | **B.1. Encriptación** | [Ver](/2backend/nestJs/B.1-encriptadoBackendNestJS.md) |
| **C.1. Backup & Restore BD** | [Ver](/2backend/nestJs/C.1-backupsRestore.md) | **D.1. Seeders** | [Ver](/2backend/nestJs/D.1-seeders.md) |
| **E.1. Crud SISPOA** | [Ver](/2backend/nestJs/E.1-generarCrudsoloSispoa.md) | **F.1. Entidades** | [Ver](/2backend/nestJs/F.1-entidadSispoa.md) |
| **G.1. Decoradores** | [Ver](/2backend/nestJs/G.1-decoradores.md) | **H.1. Consultas SQL** | [Ver](/2backend/nestJs/H.1-consultaSqlBackend.md) |
| **I.1. Linter & Formateador** | [Ver](/2backend/nestJs/I.1-lintingFormat.md) |  |  |

### 2.2. RabbitMQ

| Tema | Enlace |
|------|--------|
| **J.1. RabbitMQ (Backend)** | [Ver](/2backend/rabbitmq/J.1-rabbit.md) |
| **2.5. Node + RabbitMQ (Backend)** | [Ver](/2backend/rabbitmq/1-rabbit.md) |

### 2.3. MongoDB

| Tema | Enlace |
|------|--------|
| **K.1. MongoDb (Backend)** | [Ver](/2backend/monodb/K.1-mongodb.md) |

### 2.4. Repositorios externos (Backend)

| Proyecto | Enlace |
|----------|--------|
| Express Backend | [Ver](https://github.com/xixay/articulo-repositorio-express-backend) |
| TypeOrm Backend | [Ver](https://github.com/xixay/articulo-repositorio-typeOrm-backend) |
| NestJS Backend | [Ver](https://github.com/xixay/articulo-repositorio-NestJS-backend) |
| Node + MongoDB | [Ver](https://github.com/xixay/node-mongoDB) |
| ts2-repositorio | [Ver](https://github.com/xixay/ts2-repositorio) |
| ts-node-express-repositorio | [Ver](https://github.com/xixay/ts-node-express-repositorio) |
| typeorm-repositorio | [Ver](https://github.com/xixay/typeorm-repositorio) |
| ts1-repositorio | [Ver](https://github.com/xixay/ts1-repositorio) |
| postgres-repositorio | [Ver](https://github.com/xixay/postgres-repositorio) |
| microservicio-api-user-kong-repositorio | [Ver](https://github.com/xixay/microservicio-api-user-kong-repositorio) |
| microservicio-api-account-kong-repositorio | [Ver](https://github.com/xixay/microservicio-api-account-kong-repositorio) |
| api-kong-repositorio | [Ver](https://github.com/xixay/api-kong-repositorio) |
| microservicio-repositorio | [Ver](https://github.com/xixay/microservicio-repositorio) |
| microservicio-repositorio-cliente | [Ver](https://github.com/xixay/microservicio-repositorio-cliente) |
| microservicio-rabbitMQ-repositorio-cliente | [Ver](https://github.com/xixay/microservicio-rabbitMQ-repositorio-cliente) |

---

## 3. 🎨 Frontend

### 3.1. Formularios HTML

| Tema | Enlace |
|------|--------|
| **3.1. Formulario Html** | [Ver](https://github.com/xixay/Formulario-Repositorio) |

### 3.2. Vuetify

| Tema | Enlace |
|------|--------|
| **3.2.1. Vuetify (Intro)** | [Ver](/3frontend/vuetify/1-vuetify.md) |
| **3.2.2. Instalando Vue y Vuetify** | [Ver](/3frontend/vuetify/1.1-instalar.md) |
| **3.2.3. Componente Vuetify** | [Ver](/3frontend/vuetify/1.2-componente.md) |
| **3.2.4. Lienzo Vuetify** | [Ver](/3frontend/vuetify/1.3-lienzo.md) |
| **3.2.5. Select Vuetify** | [Ver](/3frontend/vuetify/1.4-select.md) |
| **3.2.6. Estilos con Variables Vuetify** | [Ver](/3frontend/vuetify/1.5-estilos%20como%20variables.md) |
| **3.2.7. TextField Vuetify** | [Ver](/3frontend/vuetify/1.6-textField.md) |
| **3.2.8. Checkbox Vuetify** | [Ver](/3frontend/vuetify/1.7-checkBox.md) |
| **3.2.9. Emit y On Vuetify** | [Ver](/3frontend/vuetify/1.8-emitYOn.md) |
| **3.2.10. Centrar contenido de una fila** | [Ver](/3frontend/vuetify/1.9-centrarFila.md) |
| **3.2.11. V-DataTable** | [Ver](/3frontend/vuetify/1.10-dataTable.md) |
| **3.2.12. Nuxt (Frontend)** | [Ver](/3frontend/vuetify/2-nuxt.md) |
| **3.2.13. Vuetify Repositorio** | [Ver](https://github.com/xixay/vuetify-repositorio) |
| **3.2.14. Vuetify3 Repositorio** | [Ver](https://github.com/xixay/vuetify3-repositorio) |


### 3.3. React + MUI

| Tema | Enlace |
|------|--------|
| **3.3. ReactMui (Intro)** | [Ver](/3frontend/reactMui/1-reactMui.md) |
| **3.3.1. Componente ReactMui** | [Ver](/3frontend/reactMui/1.1-componente.md) |

### 3.4. PrimeReact

| Tema | Enlace |
|------|--------|
| **3.4.1. Alineación** | [Ver](/3frontend/primereact/1.1alineaciones.md) |
| **3.4.2. Responsividad** | [Ver](/3frontend/primereact/1.2responsivo.md) |
| **3.4.3. Estilos** | [Ver](/3frontend/primereact/1.3estilos.md) |
| **3.4.4. Mostrar u ocultar** | [Ver](/3frontend/primereact/1.4-visibilidad.md) |
| **3.4.5. Select** | [Ver](/3frontend/primereact/1.5-select.md) |
| **3.4.6. Prime-Flex (Repositorio / Documentación)** | [Ver](https://github.com/xixay/prime-flex) |


## 4. 🛠️ Utilitarios

| Tema | Enlace |
|------|--------|
| **4.1. Array** | [Ver](/4utilitarios/1-array.md) |
| **4.2. Formulario (Html)** | [Ver](/4utilitarios/2-formularioHtml.md) |
| **4.3. Formulario 2 (Html)** | [Ver](/4utilitarios/3-formularioHtml2.md) |

---

## 5. 🎓 Cursos

| Tema | Enlace |
|------|--------|
| **5.1. React y Next_14(Incompleto)** | [Ver](https://github.com/xixay/React_y_Next_14) |
| **5.2. Vue y JS** | [Ver](https://github.com/xixay/curso_vue) |
| **5.3. Proyecto con Vite (Sencillo)** | [Ver](https://github.com/xixay/vite-react-example) |
| **5.4. NestJS y Angular(Incompleto)** | [Ver](https://github.com/xixay/curso_nestjs_ang) |
| **5.5. React y JS(Incompleto)** | [Ver](https://github.com/xixay/curso_react) |
| **5.6. React y Nesxt y TS(Incompleto)** | [Ver](https://github.com/xixay/curso_react_next) |
| **5.7. NestJS** | [Ver](https://github.com/xixay/curso_nestjs) |
| **5.8. Angular** | [Ver](https://github.com/xixay/curso_angular) |
| **5.9. Vue TS** | [Ver](https://github.com/xixay/curso_vue_ts) |
| **5.10. Laravel(Incompleto)** | [Ver](https://github.com/xixay/curso_laravel_12) |


## 6. 🌐 Estado del Stack Tecnológico (2025)

| Tema | Enlace |
|------|--------|
| **6.1. Tecnologías con soporte y sin soporte (2025)** | [Ver](/6estado/1-stack-2025.md) |
---

## 7. 🌐 Proyectos Antigravity

| Tema | Enlace |
|------|--------|
| **7.1. Nestjs** | [Ver](https://github.com/xixay//a_curso_nest_proyecto_base_backend) |
| **7.2. Express** | [Ver](https://github.com/xixay/express_postgres) |
---