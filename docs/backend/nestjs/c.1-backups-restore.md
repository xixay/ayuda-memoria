...
arg1=${1:-pg14}
arg2=${2:-java-scriptbase_db}
arg3=${3:-java-scriptbase_db.gz}
...
```console
sudo bash create-backup.sh conNestJs articulos_db java-scriptbase_db.gz
```
## 2. restaurar
...
docker exec $dockerContainer pimagenes -h $dbhost -U postgres -c "DROP DATABASE IF EXISTS $dbname"
docker exec $dockerContainer pimagenes -h $dbhost -U postgres -c "CREATE DATABASE $dbname ENCODING 'UTF-8'"
...
echo " - Restaurando..."
docker exec $dockerContainer pimagenes -h $dbhost -U postgres -d "$dbname" -f "$filename"
sleep 1;
...
```console
sudo bash restore-backup.sh conNestJs java-scriptbase_db java-scriptbase_db.gz
```