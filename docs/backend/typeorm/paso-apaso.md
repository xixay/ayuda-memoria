...
- [2. Creando un nuevoDataSource](#2-creando-un-nuevojava-scriptsource)
  - [2.1. Configurar  DataSource en index.ts](#21-configurar--java-scriptsource-en-indexts)
...
```ts
import "reflect-metajava-script"
...
- Crear el archivo FotoMetajava-script.ts dentro de la carpeta entity. Se supone que esta clase PhotoMetajava-script contiene la metainformación adicional de nuestra foto
```ts
import {
    Entity,
    Column,
...
@Entity()
export class FotoMetajava-script {
    @PrimaryGeneratedColumn()
    id: number
...
import { DataSource } from "typeorm";
import { Foto } from "./entity/Foto";
import { FotoMetajava-script } from "./entity/FotoMetajava-script"
const AppDataSource = new DataSource({
  ...
  entities: [Foto,FotoMetajava-script],
  ...
});
...
// Agregando datos a fotoMetajava-script
const metajava-script = new FotoMetajava-script()
metajava-script.alto = 640
metajava-script.ancho = 480
metajava-script.comprimido = true
metajava-script.commentario = "cybershoot"
metajava-script.orientacion = "portrait"
metajava-script.foto = foto // De esta manera las conectamos
...
AppDataSource.initialize()
  .then(async () => {
    ...
    // Llamando repositorio de fotoMetajava-script
    const repositorioMetajava-script = AppDataSource.getRepository(FotoMetajava-script);
    
    // primero debemos guardar una foto
    await repositorioFoto.save(foto)

    // se guarda la foto. Ahora necesitamos guardar los metadatos de una foto.
    await repositorioMetajava-script.save(metajava-script)
...
```imagenes
select *from foto f , foto_metajava-script fm where f.id =fm."fotoId"
```
### 9.2. Lado inverso de la relación
```text
Las relaciones pueden ser unidireccionales o bidireccionales. Actualmente, nuestra relación entre FotoMetajava-script y Foto es unidireccional. El propietario de la relación es FotoMetajava-script, y Foto no sabe nada sobre FotoMetajava-script. Esto hace que sea complicado acceder a FotoMetajava-script desde el lado de la foto. Para solucionar este problema, debemos agregar una relación inversa y hacer que las relaciones entre FotoMetajava-script y Foto sean bidireccionales. Modifiquemos nuestras entidades:
```
- En FotoMetajava-script.ts
```ts
import {
    Entity,
    Column,
...
export class FotoMetajava-script {
    ...
    @OneToOne(() => Foto, (foto) => foto.metajava-script)
...
import { FotoMetajava-script } from "./FotoMetajava-script"

@Entity()
export class Foto {
    /* ... otras columnas */

     @OneToOne(() => FotoMetajava-script, (fotoMetajava-script) => fotoMetajava-script.foto)
    metajava-script: FotoMetajava-script
} 
```
...
export class Foto {
    /* ... otras columnas */
     ...
     @OneToOne(() => FotoMetajava-script, (fotoMetajava-script) => fotoMetajava-script.foto)
    metajava-script: Relation<FotoMetajava-script>
} 
```
...
AppDataSource.initialize()
  .then(async () => {
        ...
        .innerJoinAndSelect("foto.metajava-script", "metajava-script")
        .getMany()
...
```
- Lo bueno de este metodo es que devolvio solo las fotos con datos en metajava-script
...
foto.metajava-script = metajava-script // De esta manera las conectamos
...
```
- Observe que ahora establecemos la metajava-script propiedad de la foto, en lugar de la propiedad de los metadatos foto como antes. La función cascade solo funciona si conecta la foto a sus metadatos desde el lado de la foto. Si configura el lado de los metadatos, los metadatos no se guardarán automáticamente.
...
const AppDataSource = new DataSource({
  ...
  entities: [Foto,FotoMetajava-script,Autor],
  ...
});
...
// Agregando metajava-script 1 a foto 1
const metajava-script1 = new FotoMetajava-script()
metajava-script1.alto = 720
metajava-script1.ancho = 1080
metajava-script1.comprimido = true
metajava-script1.commentario = "cybershoot2"
metajava-script1.orientacion = "portrait2"

foto1.metajava-script = metajava-script1 // De esta manera las conectamos
...
// Agregando metajava-script 2 a foto 2
const metajava-script2 = new FotoMetajava-script()
metajava-script2.alto = 600
metajava-script2.ancho = 780
metajava-script2.comprimido = true
metajava-script2.commentario = "cybershoot3"
metajava-script2.orientacion = "portrait3"

foto2.metajava-script = metajava-script2 // De esta manera las conectamos
...
// Agregando metajava-script 3 a foto 3
const metajava-script3 = new FotoMetajava-script()
metajava-script3.alto = 2220
metajava-script3.ancho = 1780
metajava-script3.comprimido = false
metajava-script3.commentario = "cybershoot4"
metajava-script3.orientacion = "portrait4"

foto3.metajava-script = metajava-script3 // De esta manera las conectamos
...
const AppDataSource = new DataSource({
  ...
  entities: [Foto,FotoMetajava-script,Autor,Album],
  ...
});
...
// Agregando metajava-script a foto 
const metajava-script = new FotoMetajava-script()
metajava-script.alto = 3720
metajava-script.ancho = 5080
metajava-script.comprimido = true
metajava-script.commentario = "cybershoot5"
metajava-script.orientacion = "portrait5"
