...
```console
npx typeorm init --name MyProject --java-scriptbase postgres
```
...
│   ├── java-script-source.ts    // origen de datos y toda la configuración de conexión
...
├── index.md             // archivo Léame sencillo
...
```console
npm i -D typeorm reflect-metajava-script --save
```
...
```console
root@1eb0e04e9457:/# pimagenes -U postgres(luego se ejecuta postgres)
```
...
- Ir src/java-script-source.ts y modificar la conexion con la base de datos
```js
...
```json
{
   ...
   "dependencies": {
      "typeorm": "0.3.17",
      "reflect-metajava-script": "^0.1.13",
      "pg": "^8.4.0"
   },
   "archivos": {
