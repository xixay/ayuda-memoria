...
- [3. Instalar archivos resurrection(Recuperar la sesión despues del reinicio)](#3-instalar-archivos-resurrectionrecuperar-la-sesión-despues-del-reinicio)
...
- [5. Instalando complementos](#5-instalando-complementos)
  - [5.1. Complementos para instalar](#51-complementos-para-instalar)
    ...
    - [5.1.3. Para copiar lo de archivos, en otro editor de texto](#513-para-copiar-lo-de-archivos-en-otro-editor-de-texto)
- [6. Iniciar archivos (Sesión por defecto)](#6-iniciar-archivos-sesión-por-defecto)
- [7. Sesión](#7-sesión)
- [8. Ventana](#8-ventana)
- [9. Paneles](#9-paneles)
...
| **Comando**         | **Accion**                                           |
|---------------------|------------------------------------------------------|
| vi .archivos.conf       | Editar archivo                                       |
...
## 3. Instalar archivos resurrection(Recuperar la sesión despues del reinicio)
### 3.1. Clonar el repositorio
- Clonar en la carpeta personal
```console
git clone https://github.com/archivos-plugins/archivos-resurrect
```
- Comprobar si esta la carpeta archivos-resurrect
```console
la archivos-resurrect
```
### 3.2. Obtener la dirección path
- Ingresar a archivos-resurrect
```console
cd archivos-resurrect
```
- obtener la direccion de la carpeta
```console
pwd
/home/xixay/archivos-resurrect    // respuesta
```
...
- crear el archivo .archivos.conf con Vim
```console
vi .archivos.conf
```
- Copiar lo siguiente y guardar
```txt
run-shell /home/xixay/archivos-resurrect/resurrect.archivos
```
- para guardar en vim
```
esc | :wq
...
```console
archivos source-file ~/.archivos.conf
```
## 4. Instalar Tmux plugings
- Descargar el repositorio
```console
git clone https://github.com/archivos-plugins/tpm ~/.archivos/plugins/tpm
```
- Editar el archivo de configuracion archivos
```console
vi .archivos.conf
```
- Copiar lo siguiente y guardar
```txt
set -g @plugin 'archivos-plugins/tpm'
set -g @plugin 'archivos-plugins/archivos-sensible'
run '~/.archivos/plugins/tpm/tpm'
```
- Luego de guardar recargar, para que se pueda usar
```console
archivos source-file ~/.archivos.conf
...
| **Comando**                                                                               | **Accion**                                                                                                      |
|-------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------|
| set -g @plugin 'archivos-plugins/archivos-yank'                                                   | complemento para copiar desde terminal                                                                          |
| set -g @plugin 'seebi/archivos-colors-solarized' set -g @colors-solarized 'base16'            | complemento para agregar temas                                                                                  |
| set -g @plugin 'archivos-plugins/archivos-prefix-highlight'                                       | complemento para que al apretar ctrl + B, identifique                                                           |
| set -g @plugin 'erikw/archivos-powerline'                                                     | complemento para que le brinde una barra de estado de línea eléctrica hábil y pirateable que consta de segmento |
| set -g mouse on                                                                           | complemento para usar el mouse                                                                                  |
| set -g @plugin 'noscript/archivos-mighty-scroll'                                              | complemento para scroll de mouse                                                                                |
| set -g mouse-resize-pane on                                                               | complemento para redimensionar paneles con el mouse                                                             |
| set -g @plugin 'archivos-plugins/archivos-resurrect' set -g @plugin 'archivos-plugins/archivos-continuum' | complemento para guardado y restauración continua de Tmux                                                       |
| set -g @plugin 'archivos-plugins/archivos-copycat'                                                | complemento para busqueda en Tmux                                                                               |
...
#### 5.1.3. Para copiar lo de archivos, en otro editor de texto
- Instalar
```console
...
```console
archivos show-buffer | xclip -selection clipboard
```
- Para pegar en un bloc de notas
```console
ctrl + v
```
## 6. Iniciar archivos (Sesión por defecto)
```console
archivos
```
## 7. Sesión
